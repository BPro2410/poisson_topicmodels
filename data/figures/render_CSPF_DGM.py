...
def draw_node(x, y, label, style="latent", r=R):
    ...
    if style == "det":
        ...
        ax.add_patch(circ2)
    ax.text(x, y, label, ha="center", va="center", fontsize=9, zorder=5, math_fontfamily="cm")
...
def arrow(x1, y1, x2, y2, r_from=R, r_to=R, shrink_extra=0.02):
    ...
    ax.annotate(
        ...
        xy=(ex, ey),
        xytext=(sx, sy),
        arrowprops=dict(arrowstyle="->,head_width=0.18,head_length=0.14", lw=1.3, color="black"),
        zorder=3,
    )
...
def draw_plate(x, y, w, h, label, color, label_pos="se"):
    ...
    ax.text(
        tx, ty, label, fontsize=10, color=color, ha=ha, va="bottom", fontstyle="italic", zorder=2
    )
...
draw_plate(-4.8, 0.8, 16.0, 10.4, r"$k = 1, \ldots, K$", "darkorange", "nw")  # Topic plate
# g plate: groups — contains ρ^(δ), δ², λ_{gk}, X_g
draw_plate(1.7, 5.2, 5.8, 5.8, r"$g = 1, \ldots, G$", "royalblue", "se")  # Group plate
# d plate: documents — contains x_d, μ, θ, y_{dv}
draw_plate(-3.8, -0.7, 9.3, 4.8, r"$d = 1, \ldots, D$", "forestgreen", "sw")  # Document plate
# v plate: vocabulary — contains β*, β̃, β, y_{dv}
draw_plate(3.2, -0.7, 7.3, 6.0, r"$v = 1, \ldots, V$", "firebrick", "se")  # Vocabulary plate

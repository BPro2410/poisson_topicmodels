...
![Unsupervised topic modeling](pf.png)
...
Before interpreting topics, check whether training actually converged.
In SPF, the optimization target is the negative ELBO. In practice, you want to see:

- a clear downward trend at the beginning,
- then a gradual flattening as training stabilizes.

If the loss is highly unstable or still decreasing sharply at the end, the model likely needs more training steps or different hyperparameters (for example, a smaller learning rate).

You can inspect the loss directly after training:

```python
# After model training, plot full and smoothed loss curves
fig, axes = model.plot_model_loss(window=20, save_path="loss.png")

# Optional: inspect raw values for quick diagnostics
print(f"First loss: {model.Metrics.loss[0]:.3f}")
print(f"Last loss:  {model.Metrics.loss[-1]:.3f}")
```
